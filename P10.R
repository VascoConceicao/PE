...
# Calculate the midpoint (mode of the symmetric triangular distribution)
mode <- (a + b) / 2

# Define the breaks based on k intervals
breaks <- seq(a, b, length.out = k + 1)

# Compute observed frequencies
observed <- hist(data, breaks = breaks, plot = TRUE)$counts

# Compute expected frequencies based on the triangular distribution
expected <- numeric(k)
total_area <- 0
for (i in 1:k) {
  lower <- breaks[i]
  upper <- breaks[i + 1]
  if (upper <= mode) {
    expected[i] <- ((upper - a)^2 - (lower - a)^2) / (2 * (mode - a))
  } else {
    expected[i] <- ((b - lower)^2 - (b - upper)^2) / (2 * (b - mode))
  }
  total_area <- total_area + expected[i]
}

# Normalize expected frequencies to sum to n
expected <- expected / total_area * n
